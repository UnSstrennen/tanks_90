...
class Player:
    def __init__(self, pos):
        self.x, self.y = pos
        self.go = 'up'
        self.player_image = pygame.image.load('tank_up.png').convert_alpha()
...
class Bullet:
    def __init__(self, x, y, nx, ny, ind):
        ...
        self.player_image = pygame.image.load('fire.png').convert_alpha()
...
class Enemy(Player):
    def __init__(self, x, y):
        self.x, self.y = x, y
        self.nx, self.ny = 0, 5
        self.player_image = pygame.image.load('enemy_down.png').convert_alpha()
...
while run:
    ...
    if keys[pygame.K_LEFT]:
        player.img('tank_left.png')
        player.move(-SPEED, 0)

    elif keys[pygame.K_RIGHT]:
        player.img('tank_right.png')
        player.move(SPEED, 0)

    elif keys[pygame.K_UP]:
        player.img('tank_up.png')
        player.move(0, -SPEED)

    elif keys[pygame.K_DOWN]:
        player.img('tank_down.png')
        player.move(0, SPEED)
